...
OUT_CONFIG_DIR="config/generated/btc_edge_filter_retune"
OUT_REPORT_DIR="reports/btc_edge_filter_retune"
mkdir -p "$OUT_CONFIG_DIR" "$OUT_REPORT_DIR"

# Retune only strategies that produced some raw gross signal in the base matrix.
# Goal: stop overtrading and see whether stricter edge filters can survive taker fee.
strategies=(
...
write_config() {
  ...
  case "$profile" in
    edge50_rr15_cd4) min_reward=50.0; min_net=25.0; rr=1.5; cooldown=4; tp=1.5; sl=1.0; min_vol=1.0; strict=false ;;
    edge80_rr18_cd8) min_reward=80.0; min_net=50.0; rr=1.8; cooldown=8; tp=1.8; sl=1.0; min_vol=1.1; strict=true ;;
    edge120_rr20_cd12) min_reward=120.0; min_net=80.0; rr=2.0; cooldown=12; tp=2.2; sl=1.0; min_vol=1.2; strict=true ;;
    *) echo "unknown profile: $profile" >&2; exit 1 ;;
  esac

  local name="${strategy}_${tf}_${profile}"
...
summary="$OUT_REPORT_DIR/retune_summary.csv"
echo "strategy_id,timeframe,profile,total_trades,win_rate,net_pnl,gross_pnl,total_fee,total_slippage,profit_factor,expectancy,max_drawdown,max_consecutive_losses,reports_dir" > "$summary"

for strategy in "${strategies[@]}"; do
  for tf in "${timeframes[@]}"; do
    for profile in "${edge_profiles[@]}"; do
      cfg="$(write_config "$strategy" "$tf" "$profile")"
      report="$(awk -F '"' '/reports_dir/ {print $2}' "$cfg" | tail -n 1)"
      echo
      echo "================================================================="
      echo "Retune $strategy $tf $profile"
      echo "================================================================="
      rm -rf "$report"
      cargo run --release -- research --config "$cfg"
      test -f "$report/backtest_summary.json"
      python3 - "$strategy" "$tf" "$profile" "$report" "$summary" <<'PY'
import csv, json, sys
strategy, tf, profile, report, summary = sys.argv[1:6]
