...
pub fn regression_metrics(p: &[Prediction]) -> RegressionMetrics {
    ...
    let mae = p
        .iter()
        .map(|x| (x.predicted_bps - x.actual_bps).abs())
        .sum::<f64>()
        / n;
    let rmse = (p
        .iter()
        .map(|x| (x.predicted_bps - x.actual_bps).powi(2))
        .sum::<f64>()
        / n)
        .sqrt();
    let ap = p.iter().map(|x| x.predicted_bps).sum::<f64>() / n;
    let aa = p.iter().map(|x| x.actual_bps).sum::<f64>() / n;
    let ac = p.iter().map(|x| x.actual_after_cost_bps).sum::<f64>() / n;
    let cov = p
        .iter()
        .map(|x| (x.predicted_bps - ap) * (x.actual_bps - aa))
        .sum::<f64>();
    let vp = p
        .iter()
        .map(|x| (x.predicted_bps - ap).powi(2))
        .sum::<f64>();
    let va = p.iter().map(|x| (x.actual_bps - aa).powi(2)).sum::<f64>();
    ...
    let da = p
        .iter()
        .filter(|x| (x.predicted_bps >= 0.0) == (x.actual_after_cost_bps >= 0.0))
        .count() as f64
        / n;
    RegressionMetrics {
        mae,
