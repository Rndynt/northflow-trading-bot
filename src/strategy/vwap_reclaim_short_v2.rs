//! VwapReclaimShortV2 — lookback-aware bearish breakdown/retest strategy.
//!
...
use crate::config::VwapReclaimShortV2Config;
...
#[derive(Debug, Clone)]
pub struct VwapReclaimShortV2 {
    pub cfg: VwapReclaimShortV2Config,
}

impl VwapReclaimShortV2 {
    pub fn new(cfg: VwapReclaimShortV2Config) -> Self {
        Self { cfg }
    }
}

impl Strategy for VwapReclaimShortV2 {
    ...
    fn evaluate(
        ...
        let anchor_range_high = anchor_range
            .iter()
            .map(|c| c.high)
            .fold(f64::NEG_INFINITY, f64::max);
        let anchor_range_low = anchor_range
            .iter()
            .map(|c| c.low)
            .fold(f64::INFINITY, f64::min);
        let anchor_range_width_atr = (anchor_range_high - anchor_range_low) / atr;
        if anchor_range_width_atr > self.cfg.max_anchor_range_atr {
            return Ok(None);
        }

        let Some(breakdown) = recent_window
            .iter()
            .rev()
            .find(|c| c.close < anchor_range_low)
        else {
            return Ok(None);
        };
        let breakdown_close_atr = (anchor_range_low - breakdown.close) / atr;
        if breakdown_close_atr < self.cfg.min_breakdown_close_atr
            || breakdown_close_atr > self.cfg.max_breakdown_close_atr
        {
            return Ok(None);
        }
        if self.cfg.require_breakdown_volume_expansion {
            let breakdown_volume_ratio = breakdown.volume / volume_sma_20;
            if breakdown_volume_ratio < self.cfg.min_breakdown_volume_ratio {
                return Ok(None);
            }
        }

        let candle = input.entry_candle;
        let bearish_retest_hold = candle.high
            >= anchor_range_low - self.cfg.retest_tolerance_atr * atr
            && candle.close < anchor_range_low
        ...
        let body_ratio = if candle_range > 0.0 {
            (candle.open - candle.close).abs() / candle_range
        } else {
            0.0
        };
        if self.cfg.require_current_bearish_body && body_ratio < self.cfg.min_current_body_ratio {
            return Ok(None);
        }

        let upper_wick_ratio = if candle_range > 0.0 {
            (candle.high - candle.open.max(candle.close)) / candle_range
        } else {
            0.0
        };
        if upper_wick_ratio < self.cfg.min_upper_wick_rejection_ratio {
            return Ok(None);
        }

        let extension_atr = (anchor_range_low - candle.close) / atr;
        ...
        let recent_window_high = recent_window
            .iter()
            .map(|c| c.high)
            .fold(f64::NEG_INFINITY, f64::max);
        let stop_loss = match self.cfg.stop_mode.as_str() {
            "atr" => entry + atr * self.cfg.sl_atr_multiple,
            "structure" => {
                let structure_stop = candle.high.max(recent_window_high).max(anchor_range_low)
                    + atr * self.cfg.structure_stop_buffer_atr;
                if (structure_stop - entry) / atr > self.cfg.max_structure_stop_atr {
                    return Ok(None);
                }
                structure_stop
            }
        ...
        Ok(Some(Signal {
            ...
            strategy_id: StrategyId::new("vwap_reclaim_short_v2"),
            side: Side::Short,
            ...
            entry_reason: "vwap_reclaim_short_v2_range_breakdown_retest".to_string(),
            filters_passed: vec![
                "screening_bearish".to_string(),
                "confirmation_bearish".to_string(),
                "entry_ema_alignment_short".to_string(),
                "anchor_range_quality_ok".to_string(),
                "breakdown_close_depth_ok".to_string(),
                "breakdown_volume_ok".to_string(),
                "retest_hold_ok".to_string(),
                "current_body_ok".to_string(),
                "upper_wick_rejection_ok".to_string(),
                "structure_stop_ok".to_string(),
...
mod tests {
    ...
    fn strategy() -> VwapReclaimShortV2 {
        VwapReclaimShortV2::new(VwapReclaimShortV2Config {
            lookback_bars: 5,
    ...
    fn lookback() -> Vec<Candle> {
        vec![
            c(1, 101.0, 101.5, 100.0, 101.0),
            c(2, 101.0, 101.5, 100.5, 101.0),
            c(3, 101.0, 101.5, 100.2, 101.0),
            c(4, 101.0, 101.5, 100.1, 101.0),
            c(5, 101.0, 101.5, 100.3, 101.0),
            c(6, 100.0, 100.1, 99.8, 99.9),
            c(7, 99.9, 100.1, 99.5, 99.8),
            c(8, 99.8, 100.1, 99.4, 99.7),
        ]
    }
    fn input() -> MultiTimeframeInput {
        MultiTimeframeInput {
            entry_candle: c(9, 100.0, 100.1, 99.7, 99.85),
    ...
    fn strategy_id_is_stable() {
        assert_eq!(strategy().strategy_id(), "vwap_reclaim_short_v2");
    }
    #[test]
    ...
    #[test]
    fn no_signal_if_anchor_range_too_wide() {
        let mut i = input();
        i.entry_lookback[0].high = 103.0;
        assert!(strategy().evaluate(&ctx(), &i).unwrap().is_none());
    }
    #[test]
    fn no_signal_if_breakdown_depth_too_shallow() {
        let mut i = input();
        i.entry_lookback[7].close = 99.98;
        assert!(strategy().evaluate(&ctx(), &i).unwrap().is_none());
    }
    #[test]
    fn no_signal_if_breakdown_depth_too_extended() {
        let mut i = input();
        i.entry_lookback[7].low = 98.9;
        i.entry_lookback[7].close = 99.0;
        assert!(strategy().evaluate(&ctx(), &i).unwrap().is_none());
    }
    #[test]
    fn no_signal_if_breakdown_volume_required_but_absent() {
        let mut i = input();
        i.entry_lookback[7].volume = 100.0;
        assert!(strategy().evaluate(&ctx(), &i).unwrap().is_none());
    }
    #[test]
    fn no_signal_if_structure_stop_is_too_wide() {
        let mut s = strategy();
        s.cfg.max_structure_stop_atr = 1.0;
        assert!(s.evaluate(&ctx(), &input()).unwrap().is_none());
    }
    #[test]
